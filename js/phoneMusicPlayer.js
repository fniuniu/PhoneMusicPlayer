...
var nameDisplay = function() {
    ...
    $('#id-song-title').text(name)
	$('#id-singer').text(songs[name])
}
//根据传入的song进行播放
...
var playSong = function(button) {
	...
	$('.triangle-c').addClass('off')
	$('.rectangle').removeClass('off')
	nameDisplay()
}
//暂停按钮
var pauseSong = function(button) {
	...
	$('.rectangle').addClass ('off')
	$('.triangle-c').removeClass('off')
}
//绑定三个控制按钮
...
var createHoverState = function(myobject){
  ...
  var setSliderValue = function(value) {
  	var time = value * 100
  	$( ".green" ).slider( "option", "value", time )
  }
  //补全时间
  ...
  var bindTimeSlider = function() {
  	...
  	player.on('timeupdate', function(event) {
  		...
  		$('.green').valueTime = self.currentPlaybackTime
  ...
  var bindAudioSlider = function() {
  	console.log('音乐跟随滑条1')
  	$('.green').on('mousedown', function(event, ui) {
  ...
  var bindAudioVolume = function() {
	  $('.white').on('mousedown', function(event, ui) {
    		var self = $(event.currentTarget)
    		console.log('self', self)
    		var value = self.slider("value")
    		console.log('value:', value, typeof(value))
